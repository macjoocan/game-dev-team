...
```
- 에이전트는 서로 직접 대화하지 않는다. 오케스트레이터가 다음 역할로 전달한다.
...
1. **기획/밸런스 + 게임성 지표 정의** (`game-designer`)
   - 게이트: 근거 없는 수치 0 · 미결 명시 · 핵심 수치 공란 0 · 게임성 성공 지표 숫자로 정의.
2. **태스크 분해** (`pm`).
3. **프로토타입(스파이크)** (`developer`) — 버려도 되는 최소 구현. 핵심 재미만 싸게.
4. **게임성 검증** (`game-designer` + `balance-sim`)
   - 게이트: 지표 충족. **미충족 시 1번으로 되돌려 재조정(반복 루프).**
5. **본구현** (`developer`) — worktree 브랜치, 작은 커밋.
6. **정확성 검증** (`qa`) — 버그·회귀·콘솔 에러.
7. **아트** (`artist`) — 요청서 → 시안 → 승인.

> **3↔4는 반복 루프다.** 재미가 안 나오면 5로 넘어가지 않는다. 반복 예산을 정해 무한루프를 막는다.
